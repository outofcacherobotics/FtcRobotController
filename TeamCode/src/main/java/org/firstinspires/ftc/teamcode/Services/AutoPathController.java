...
public class AutoPathController {
    ...
    static final double RIGHT_DRIVE_SPEED = 0.6;
    static final double LEFT_DRIVE_SPEED = 0.3;
    static final double RIGHT_TURN_SPEED = 0.4;
    static final double LEFT_TURN_SPEED = 0.2;
    ...
    public void drive(double rightCM, double leftCM) {
        ...
        setRunToPosition();
//        runtime.reset();
        if (((rightCM < 0 && leftCM > 0) || (leftCM < 0 && rightCM > 0)) && Math.abs(rightCM) == Math.abs(leftCM)) {
            setLeftAndRightPower(LEFT_TURN_SPEED, RIGHT_TURN_SPEED);
        }
...
//        }
    ...
    public void gyroDrive(double distance) {
        distance /= LEFT_DAMPING_CONSTANT;
        verifyMovement(distance, 0);
...
//        localizer.locationHistory.pushCoords(coordinateEntry);
    ...
    public void strafe(double distance) {
        ...
        int moveCounts = (int)(distance * PULSES_PER_CM);
        // strafe wheels based on movecount
    }
    ...
    public boolean onHeading(double angle) {
        ...
        if (Math.abs(error) <= HEADING_THRESHOLD) {
            ...
            rightSpeed = RIGHT_TURN_SPEED * steer;
            leftSpeed = -(LEFT_TURN_SPEED * steer);
        }
     ...
     * @return
     */
    public boolean verifyMovement(double forwardMovement, double sidewaysMovement) {
        double xMov = 0.0;
        double yMov = 0.0;
